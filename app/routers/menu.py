...
def create_menu(menu: schemas.MenuCreate, db: Session = Depends(get_db),
    ...
    try:
        menu.created_by = current_user.fullname
...
def delete_menu(id: int, db: Session = Depends(get_db),
                current_user: int = Depends(oauth2.get_current_user)):
    menu_query = db.query(models.MenuPermission).filter(models.MenuPermission.pmsid == id)
...
def update_menu(id: int, updated_menu: schemas.MenuCreate, db: Session = Depends(get_db),
                current_user: int = Depends(oauth2.get_current_user)):
    menu_query  = db.query(models.MenuPermission).filter(models.MenuPermission.pmsid == str(id))
    menu = menu_query.first()
    if menu == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Menu with id {id} does not exist")
...
async def getmenuassignedrole(db: Session = Depends(get_db),
    ...
    menu_query = db.query(models.UspRuleNpmsAssign.pmsid,
